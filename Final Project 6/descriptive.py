...
def _dispersion(series: pd.Series):
    ...
    stdev = x.std(ddof=0)  # matches np.std default behavior
    xmin = x.min()
    xmax = x.max()
    rng = xmax - xmin
    q25 = x.quantile(0.25)
...
def plot_descriptive(num=1, jitter_scale=0.06, point_alpha=0.35, a_hspace=0.12):
    ...
    display_title("Descriptive plots", pref="Figure", num=num, center=False)
    ...
    # Panel A: 2x3 across full width (adjust hspace here)
    ...
    axs_A[0, 0].set_title("(A) Continuous predictors", loc="left")
    ...
    axs_B[0].set_title("(B) Categorical predictors", loc="left")
